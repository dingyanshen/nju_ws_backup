...
class PhotoServiceResponse(genpy.Message):
  _md5sum = "49012d3a75867c44d7c8a631f08af3cd"
  _type = "camera/PhotoServiceResponse"
  _has_header = False #flag to mark the presence of a Header object
  _full_text = """float32 error_x
float32 error_y
"""
  __slots__ = ['error_x','error_y']
  ...
  def serialize(self, buff):
    ...
    try:
      _x = self
      buff.write(_get_struct_2f().pack(_x.error_x, _x.error_y))
    except struct.error as se: self._check_types(struct.error("%s: '%s' when writing '%s'" % (type(se), str(se), str(locals().get('_x', self)))))
    except TypeError as te: self._check_types(ValueError("%s: '%s' when writing '%s'" % (type(te), str(te), str(locals().get('_x', self)))))
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      (_x.error_x, _x.error_y,) = _get_struct_2f().unpack(str[start:end])
      return self
    except struct.error as e:
      raise genpy.DeserializationError(e) #most likely buffer underfill
...
class PhotoService(object):
  ...
  _md5sum = '117e6535d65959983e49d94959e9d276'
